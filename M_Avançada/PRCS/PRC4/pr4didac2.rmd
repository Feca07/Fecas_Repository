...
\clearpage
```{r dis4,fig.cap="Balanç de covariables: Cru vs IPTW",fig.width=6,fig.height=4}
library(cobalt)
...
A la figura \ref{fig:dis4} mostra les diferències estandarditzades de mitjanes (SMD) abans i després d’aplicar l’ajust mitjançant IPTW. Els punts vermells representen l’anàlisi crua, mentre que els punts blaus representen l’anàlisi ajustada amb IPTW.
...
```{r supv,fig.cap="Supervivència de l'empelt - Ajust IPTW",fig.width=6,fig.height=6}
library(survival)
library(survminer)
library(dplyr)
...
fit_patient <- survfit(Surv(DeathTm, Death_Event) ~ treat,
                       data = data_tot, weights = IPTW)
```
```{r supv2,fig.cap="Supervivència del pacient - Ajust IPTW",fig.width=6,fig.height=6}
...
\clearpage
En les figures \ref{fig:supv} i \ref{fig:supv2}, s'observen dos elements claus:
...
```{r grafic_censures ,eval=T,echo=FALSE,results=F}
library(MatchIt)
...
# 1. Assegurem que PatNo sigui caràcter a ambdues taules per evitar l'error de tipus
data_matched <- data_matched %>% mutate(PatNo = as.character(PatNo))
outcomes <- outcomes %>% mutate(PatNo = as.character(PatNo))

# 2. Fem el join. 
# El left_join només mantindrà els pacients que estan a data_matched (els aparellats)
final_matched <- data_matched %>%
  left_join(outcomes, by = "PatNo")

# Ara ja pots procedir amb el Kaplan-Meier (Part 3.1)
# Comprova que no tinguis missings en les noves variables:
summary(final_matched$GraftNotFxTm) 
...
```{r grafic_censures2, fig.cap="Supervivència de l'empelt (PSM)",fig.width=6,fig.height=6}
...
## Identificació de les variables per a l'anàlisi inferencial
...
```
```{r ref.label='supv2', echo=TRUE, eval=FALSE}
```
## Models de Cox 
\label{cox}
...
```
```{r ref.label='grafic_censures2',echo=TRUE, eval=F}
